...
def train_agent(env, agent, n_episodes=1000, eps_start=1.0, eps_decay=0.995, eps_min=0.01):
    """The function trains an agent using DQN, Double DQN (DDQN), Dueling DQN (Dueling DQN) and
    Dueling DDQN (Dueling_DDQN) techniques. The technique can be chosen when the Agent class is called.
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        if np.mean(scores[-average_size:]) >= 13.0:
            if model_choice == "DQN":
                ...
                torch.save(agent.network_local.state_dict(), 'saved_network_Dueling_DQN.pth')
                break
            elif model_choice == "Dueling_DDQN":
                torch.save(agent.network_local.state_dict(), 'saved_network_Dueling_DDQN.pth')
                break
    return scores
