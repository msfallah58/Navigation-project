...
class Agent:
    ...
    def __init__(self, state_size, action_size, GAMMA=0.99, gae_lambda=0.95,
                 policy_clip=0.2, BATCH_SIZE=128, BUFFER_SIZE=int(1e5), n_epochs=4):
        """
    ...
    def learn(self, experiences):
        ...
        for _ in range(self.n_epochs):
            ...
            for t in range(len(rewards) - 1):
                ...
                advantage[t] = a_t
            advantage = torch.tensor(advantage).unsqueeze(dim=1).to(device)
            ...
            weighted_probs = torch.mul(advantage, prob_ratio)
            clipped = torch.clamp(prob_ratio, 1 - self.policy_clip, 1 + self.policy_clip)
            weighted_clipped_probs = torch.mul(clipped, advantage)
