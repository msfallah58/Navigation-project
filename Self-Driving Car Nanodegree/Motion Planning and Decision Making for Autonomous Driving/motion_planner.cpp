...
std::vector<State> MotionPlanner::generate_offset_goals(
  ...
  auto yaw_perpen = goal_state.rotation.yaw + M_PI/2;  // <- Fix This
  ...
  for (int i = 0; i < _num_paths; ++i) {
    ...
    goal_offset.location.x += goal_offset.location.x + cos(yaw_perpen);  // <- Fix This
    goal_offset.location.y += goal_offset.location.y + cos(yaw_perpen);  // <- Fix This
