...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = -pow(velocity_mag, 2)/(2*accel_mag);  // the distance is calculated by (v_final^2 - v_init^2)/(2*acceleration). Here v_final = 0.  
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.velocity.x = 0;  // 
      goal.velocity.y = 0;  // 
      goal.velocity.z = 0;  // There is no speed vector in z direction 
      ...
      goal.velocity.x = std::cos(goal.rotation.yaw)*_speed_limit;  // the projection of speed limit vector on x direction
      goal.velocity.y = std::sin(goal.rotation.yaw)*_speed_limit;  // the projection of speed limit vector on y direction
      goal.velocity.z = 0; // There is no speed vector in z direction
    }
