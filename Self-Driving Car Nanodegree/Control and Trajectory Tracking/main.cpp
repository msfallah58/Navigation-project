...
int main ()
{
  ...
  **/
  PID pid_steer = PID();
  pid_steer.Init(0.5, 0.05, 0.4, 1.2, -1.2);
  ...
  **/
  ...
  pid_throttle.Init(0.2, 0.05, 0.2, 1.0, -1.0);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          // it is noted that Yaw angle and steering angle are nonlnearly related. In fact yaw angle is a function of longitudinal velocity and lateral velocity as well but it is reasonable to estimate the error in steering angle using the error in Yaw angle.  
