...
// The code is mainly borrowed from Lesson 6 "Solution: NDT Alignment"
// Here we find the transformation matrix using NDT approach through scan matching
Eigen::Matrix4d NDT(PointCloudT::Ptr mapCloud, PointCloudT::Ptr source, Pose startingPose, int iterations){
  ...
  Eigen::Matrix4d tansformation_matrix = ndt.getFinalTransformation ().cast<double>();
  return tansformation_matrix;
}
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
            ...
            // The code is mainly borrowed from Lesson 6 "Solution: NDT Alignment"
          	pcl::VoxelGrid<PointT> vg; //To declare the voxel grid
          	vg.setInputCloud(scanCloud); //To set the input cloud
          	double filterRes = 0.5; //To set the filter resolution
          	vg.setLeafSize(filterRes, filterRes, filterRes); //To set the leaf size of voxel grid 
          	typename pcl::PointCloud<PointT>::Ptr CloudFiltered (new pcl::PointCloud<PointT>); //To call the cloud filtered 
          	vg.filter(*cloudFiltered); //To filter the cloud points
          

			// TODO: Find pose transform by using ICP or NDT matching
			//pose = ....
          	Eigen::Matrix4d matching_transform = NDT(mapCloud, cloudFiltered, pose, 60);
            // Geting the pose based on the matching transform.
